...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 ...
                 num_class=28, duppos_mode='zeros',
                 slot_iters=3):
        super().__init__()
        ...
        # Psychological-aware Query Generation (PQG): slot attention over the
        # psychological-text features, producing image-specific intent queries.
        self.slot_atten = SlotAttention(
    ...
    def forward(self, src, query_embed, pos_embed, cap_feats_proj, emo_feats, mask=None, use_peqg=True):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1)  # (HW, B, h_dim)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)
        if mask is not None:
            mask = mask.flatten(1)

        # emotion feature map -> token sequence
        emo_feats = emo_feats.flatten(2).permute(2, 0, 1)

        # --- Step 1: Visual-Psychological Interaction (VPI): cross-attention ---
        # Visual features attend over the psychological-text features (Eq. 1).
        src_cap = self.decoder(src, cap_feats_proj.permute(1, 0, 2)).squeeze(0)

        # --- Step 2: Transformer encoder over Cat(V_hat, E) (Eq. 2) ---
        if self.num_encoder_layers > 0:
            src = torch.cat([src_cap, emo_feats], dim=0)
            duppos = torch.cat([pos_embed, torch.zeros_like(pos_embed) if self.duppos_mode == "zeros" else pos_embed], dim=0)
            memory = self.encoder(src, src_key_padding_mask=mask, pos=duppos)[:emo_feats.size(0)]
        else:
            memory = src_cap

        # --- Step 3: Psychological-aware Query Generation (PQG) ---
        if use_peqg:
            tgt = self.slot_atten(cap_feats_proj, mask)
            tgt = tgt.permute(1, 0, 2)  # [#queries, bsz, d_model]
        else:
            tgt = query_embed  # fall back to learnable class queries

        # --- Step 4: Intent decoding ---
        hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed)

        return hs.transpose(1, 2), memory[:h*w].permute(1, 2, 0).view(bs, c, h, w), cap_feats_proj.mean(1), memory.mean(0)
...
class SlotAttention(nn.Module):
    """Psychological-aware Query Generation (PQG).

    The softmax is taken over the ``numc`` slot axis so that intent slots compete
    for each text token, and an L1 normalization is applied over the ``N`` token
    axis (Slot Attention's weighted-mean aggregation). The slot update first
    absorbs the aggregated cue through an additive residual, then stacks an MLP
    residual on the LayerNorm of that intermediate state (Eq. 7).
    """
    ...
    def __init__(self, num_iterations, num_slots, d_model, epsilon=1e-8):
        ...
        Args:
            ...
            num_slots: Number of slots (numc intent queries).
            d_model: Hidden layer size.
    ...
    def forward(self, inputs, mask=None):
        ...
        for _ in range(self.num_iterations):
            ...
            # softmax over the numc slot axis, L1 normalize over the N token axis
            attn = dots.softmax(dim=1)
            attn = attn / (attn.sum(dim=-1, keepdim=True) + self.epsilon)
            updates = torch.einsum('bjd,bij->bid', v, attn)  # [bsz, num_slots, d_model]

            # Slot update (Eq. 7): additive residual of the aggregated cue, then
            # an MLP residual on the LayerNorm of the intermediate state.
...
def build_transformer(args):
    return Transformer(
        ...
        duppos_mode=args.duppos_mode if hasattr(args, "duppos_mode") else "zeros",
        slot_iters=getattr(args, 'slot_iters', 3),
    )
